...
def create_dataframes(spark: SparkSession) -> Dict[str, DataFrame]:
    ...
    # Create the DataFrames with column names and data
    ...
    rating = spark.createDataFrame(
        [
            (
                2,
                "2022-08-29T12:48:02.846Z",
                "2023-04-17T03:46:37.654Z",
                2,
                3,
                """Iste laudantium totam recusandae officia enim ea. Temporibus eos expedita \
                mollitia. Minus atque ipsam odit.""",
            )
        ],
    ...
    councillor = spark.createDataFrame(
        [
            (
                0,
                "2022-09-19T22:43:38.171Z",
                "2023-06-05T13:56:56.296Z",
                "Anxiety",
                """Dolor vitae eius voluptate eaque vero quam. Similique ullam reiciendis
                cum pariatur iusto omnis. Accusantium molestiae excepturi tempora nesciunt
                  ratione.""",
                0,
            )
        ],
    ...
    price_log = spark.createDataFrame(
        [
            (
                0,
                "2022-12-31T09:14:54.666Z",
                "2023-05-27T11:17:59.013Z",
                2019,
                "bank_transfer",
                True,
                0,
...
class TransformDataTestCase(unittest.TestCase):
    ...
    def test_load_dataframes_200_response(self):
        with patch.dict(
            os.environ, {"URL_LINK": "https://xloop-dummy.herokuapp.com/"}
            ...
            # Call the function
            result = transform_data.load_data_frames(
                self.end_points_list, data_frames, self.spark_session_mock
            )
    ...
    def test_dataframes_already_exists(self):
        with patch.dict(
            os.environ, {"URL_LINK": "https://xloop-dummy.herokuapp.com/"}
        ), patch("requests.get") as requests_mock, patch(
            ...
            existing_data = [
                (
                    0,
                    "2022-10-03T02:25:26.383Z",
                    "2023-01-01T16:18:44.590Z",
                    2209,
                    5532,
                    True,
                )
            ]
            df = self.spark_session_mock.createDataFrame(
                existing_data,
                [
                    "id",
                    "created",
            ...
            # Call the function
            result = transform_data.load_data_frames(
                end_points_list, data_frames, self.spark_session_mock
            )
    ...
    def test_rename_dfs(self):
        ...
        renamed_data_frames = transform_data.rename_columns_names(
            self.end_points_list, data_frames
        )
        ...
        assert (
            ...
            in renamed_data_frames["patient_councillor_df"].columns
        )
    ...
    def test_success_rate(self):
        ...
        expected_columns = ["councillor_id", "success_rate"]

        expected_data = [(1360, 33.335)]
    ...
    def test_duration(self):
        ...
        expected_columns = ["councillor_id", "avg_duration_per_treatment"]

        expected_data = [(1360, 60.0)]
    ...
    def test_cost(self):
        ...
        expected_columns = ["councillor_id", "avg_cost_per_treatment"]

        expected_data = [(1360, 4420.0)]
    ...
    def test_appointments_per_treatment(self):
        ...
        expected_columns = ["councillor_id", "avg_appointments_per_treatment"]

        expected_data = [(1360, 2.0)]
    ...
    def test_outcome_prediction(self):
        ...
        expected_data = [(1360, 33.335, 60.0, 4420.0, 2.0)]
